...
public static class Program
{
    ...
    private static string ReadFileWithRetry(string path, int maxRetries = 5, int baseDelayMs = 100)
    {
        for (int attempt = 0; ; attempt++)
        {
            ...
            {
                Task.Delay(baseDelayMs * (attempt + 1)).GetAwaiter().GetResult();
            }
        }
    }
    ...
    public static async Task<int> Main(string[] args)
    {
            ...
            for (int i = 1; i < args.Length; i++)
            {
                if (args[i].StartsWith("-"))
                {
                    if (
                        ...
                        || args[i].Equals("--host", StringComparison.OrdinalIgnoreCase)
        ...
        switch (command)
        {
            ...
            case "serve":
                return await ExecuteServeAsync(projectDir, configPath, port, host, watch);
    ...
    private static void PrintUsage()
    {
        ...
        Console.WriteLine("  --host, -b <h>   Specify the host to bind to (default: localhost).");
        Console.WriteLine(
    ...
    )
    {
        ...
        try
        {
            ...
            {
                var json = ReadFileWithRetry(configPath);
    ...
    )
    {
        ...
        try
        {
            ...
            var server = new DevServer(outputDir, port, host);
            server.Start();
    ...
    private static void StartWatching(string projectDir, string configPath, ILogger logger)
    {
        ...
        void OnChanged(object sender, FileSystemEventArgs e)
        {
            ...
            lock (rebuildLock)
            {
                if (DateTime.UtcNow - lastRebuild < TimeSpan.FromMilliseconds(1000))
                {
                    return;
                }
            ...
            Task.Run(async () =>
            {
                // Wait for the file write to finish (editors write in multiple passes)
